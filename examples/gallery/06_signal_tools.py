...
plt.errorbar(x[::32], y[::32], yerr=0.18 + 0.05 * np.sin(x[::32] / 9), fmt="ko", ecolor="tab:red", capsize=0.05, label="sampled uncertainty")
plt.stem(events, y[events] - 1.2, linefmt="C2-", markerfmt="C2o", basefmt="k-", bottom=-1.5, label="detected events")
plt.annotate("largest spike", xy=(events[np.argmax(y[events])], np.max(y[events])), xytext=(900, 3.8), arrowprops={"color": "tab:red", "width": 1.1}, color="tab:red")
plt.xlabel("sample")
plt.ylabel("value")
