...
29. `29_colorbar_gallery.py` - `colorbar()` on all four artist types (scatter, a hand-colored line family, contour, imshow), at all four positions (default right, plus left/top/bottom), across four `Normalize` subclasses (linear, `LogNorm`, `SymLogNorm`, `PowerNorm`): a star cluster, radioactive-decay curves, a two-point-charge equipotential field, and a diffraction pattern.
