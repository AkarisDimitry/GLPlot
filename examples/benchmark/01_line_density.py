...
def main():
    ...
    for N in line_counts:
        ...
        try:
            ...
            ax.imshow(np.log1p(density), extent=(x_range[0], x_range[1], ymin_v, ymax_v),
                      origin="lower", aspect="auto", cmap="magma", interpolation="bilinear")
    ...
    try:
        ...
        valid_lc_idx = [i for i, t in enumerate(line_counts) if i < len(glplot_times) and glplot_times[i] is not None]
        valid_mpl_l_idx = [i for i, t in enumerate(line_counts) if i < len(mpl_lines_times) and mpl_lines_times[i] is not None]
        valid_mpl_d_idx = [i for i, t in enumerate(line_counts) if i < len(mpl_density_times) and mpl_density_times[i] is not None]

        ax.plot([line_counts[i] for i in valid_lc_idx], [glplot_times[i] for i in valid_lc_idx], 
                "o-", color="#1f77b4", linewidth=2.5, label="GLPlot GPU Density")
        
        ax.plot([line_counts[i] for i in valid_mpl_l_idx], [mpl_lines_times[i] for i in valid_mpl_l_idx], 
                "s--", color="#d62728", linewidth=2, label="Matplotlib CPU Lines (LineCollection)")
        
        ax.plot([line_counts[i] for i in valid_mpl_d_idx], [mpl_density_times[i] for i in valid_mpl_d_idx], 
                "^--", color="#ff7f0e", linewidth=2, label="Matplotlib CPU 2D Density (Histogram)")
        ...
        for i in valid_lc_idx:
            ...
            t = glplot_times[i]
            if count >= 10000:
                ax.annotate(f"{t:.2f}s", (count, t), textcoords="offset points", xytext=(0,10), ha='center', fontsize=9, color="#1f77b4")
                
        for i in valid_mpl_d_idx:
            count = line_counts[i]
            t = mpl_density_times[i]
            if count >= 10000:
                ax.annotate(f"{t:.2f}s", (count, t), textcoords="offset points", xytext=(0,-15), ha='center', fontsize=9, color="#ff7f0e")
