...
def _measure_repeated(fn_factory, label, n_repeats=N_REPEATS, n_warmup=N_WARMUP):
    ...
    return RunStats(
        t_mean=float(np.mean(ta)),  t_std=float(np.std(ta, ddof=min(1, len(ta)-1))),
        t_min=float(np.min(ta)),    t_max=float(np.max(ta)),
        m_mean=float(np.mean(tm_arr)), m_std=float(np.std(tm_arr, ddof=min(1, len(tm_arr)-1))),
        m_min=float(np.min(tm_arr)),   m_max=float(np.max(tm_arr)),
    )
...
def run_vispy(N, a, b, colors, output_path):
    ...
    canvas = scene.SceneCanvas(show=False, size=(INSET_W, INSET_H),
                               keys='interactive', bgcolor='black')
    view = canvas.central_widget.add_view()
    pos = np.empty((2 * N, 3), dtype=np.float32)
    pos[0::2, 0] = X_RANGE[0]; pos[0::2, 1] = a * X_RANGE[0] + b; pos[0::2, 2] = 0.0
    pos[1::2, 0] = X_RANGE[1]; pos[1::2, 1] = a * X_RANGE[1] + b; pos[1::2, 2] = 0.0
...
def run_fastplotlib(N, a, b, colors, output_path):
    ...
    pos[0::3, 0] = X_RANGE[0]; pos[0::3, 1] = a * X_RANGE[0] + b; pos[0::3, 2] = 0.0
    pos[1::3, 0] = X_RANGE[1]; pos[1::3, 1] = a * X_RANGE[1] + b; pos[1::3, 2] = 0.0
    pos[2::3] = np.nan
    ...
    subplot.camera.position = ((X_RANGE[0]+X_RANGE[1])/2.0, (Y_RANGE[0]+Y_RANGE[1])/2.0, 0.0)
    temp = output_path + ".temp.png"
    fig._render(); fig.export(temp)
    if fig._output is not None: fig.close()
...
def run_datashader(N, a, b, alpha, output_path):
    ...
    pos[0::3, 0]=X_RANGE[0]; pos[0::3, 1]=a*X_RANGE[0]+b
    pos[1::3, 0]=X_RANGE[1]; pos[1::3, 1]=a*X_RANGE[1]+b
    pos[2::3] = np.nan
    alphas = np.full(3*N, alpha, dtype=np.float32); alphas[2::3] = np.nan
...
def run_hvplot(N, a, b, alpha, output_path):
    ...
    alphas = np.full(3*N, alpha, dtype=np.float32); alphas[2::3] = np.nan
    df = pd.DataFrame({'x': pos[:,0], 'y': pos[:,1], 'alpha': alphas})
    plot = df.hvplot.line(x='x', y='y', rasterize=True, aggregator=ds.sum('alpha'),
                          dynamic=False, cmap=CMAP, colorbar=False, xlim=X_RANGE, ylim=Y_RANGE)
...
def main():
    ...
    print(f"   Repeats: {N_REPEATS}  |  Warm-up: {N_WARMUP}  |  "
          f"Effective reps: {N_REPEATS - N_WARMUP}")
    print("   Metrics: wall-clock time  +  RSS delta (psutil) / tracemalloc")
    print("=" * 70)

    for name, avail in [("VisPy", vispy_available), ("fastplotlib", fastplotlib_available),
                         ("Datashader", datashader_available), ("hvPlot", hvplot_available)]:
        if not avail:
            print(f"  ⚠  {name} not available")
    print(f"  psutil: {'available (RSS)' if psutil_available else 'NOT available – using tracemalloc'}")
    ...
    for N in line_counts:
        ...
        def _glplot_density_factory(N=N, a=a, b=b, colors=colors, out=out_d):
            def _fn():
                ...
                gplt.options(density_resolution_scale=1.0, density_gain=2.0,
                             density_scheme_index=8, axis_show_grid=False,
                             axis_show_labels=False, axis_show_frame=False, light_bg_mode=False)
                gplt.plot_lines(a, b, x_range=X_RANGE, colors=colors)
                gplt.xlim(*X_RANGE); gplt.ylim(*Y_RANGE)
        ...
        def _glplot_exact_factory(N=N, a=a, b=b, colors=colors, out=out_e):
            def _fn():
                ...
                gplt.options(axis_show_grid=False, axis_show_labels=False,
                             axis_show_frame=False, light_bg_mode=False)
                gplt.plot_lines(a, b, x_range=X_RANGE, colors=colors)
                gplt.xlim(*X_RANGE); gplt.ylim(*Y_RANGE)
                gplt.show(test_mode=True)
    ...
    try:
        ...
        for label, results, fmt, color, lw in series_meta:
            ...
            ax_t.fill_between(xs, np.maximum(t_mu - t_sig, 1e-6), t_mu + t_sig,
                              alpha=0.18, color=color)
            for x, mu, sig in zip(xs, t_mu, t_sig):
                if x >= 100_000:
                    ax_t.annotate(f"{mu:.2f}±{sig:.2f}s", (x, mu),
            ...
            for x, mu, sig in zip(xs, m_mu, m_sig):
                if x >= 100_000:
                    ax_m.annotate(f"{_fmt_mem(mu)}±{sig:.0f}", (x, mu),
                                  textcoords="offset points", xytext=(0, 10),
                                  ha='center', fontsize=6.5)
        ...
        ax_t.set_title(f"Rendering Time  (10³ → 10⁷)\nmean ± σ over {N_REPEATS-N_WARMUP} reps",
        ...
        ax_m.set_title(f"Memory Usage  (10³ → 10⁷)\nmean ± σ over {N_REPEATS-N_WARMUP} reps",
                       fontsize=13, fontweight="bold")
