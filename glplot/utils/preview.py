...
def render_preview(engine, filename: str, scale: float = 1.0) -> None:
    ...
    has_3d = any(layer.layer_type in {"scatter3d", "mesh3d", "wireframe3d", "bars3d", "volume3d"} for layer in engine.scene.layers)
    if has_3d:
    ...
    for layer in engine.scene.layers:
        ...
        if layer.layer_type in {"scatter3d", "volume3d"} and layer.vertices is not None:
            ...
            ax.scatter(verts[:, 0], verts[:, 1], verts[:, 2], c=layer.colors, s=max(layer.style.point_size, 0.5), depthshade=False, label=layer.label or None)
            continue
        if layer.layer_type == "wireframe3d" and layer.vertices is not None:
            verts = layer.vertices.reshape(-1, 2, 3)
            color = _rgba(layer.colors[0] if layer.colors is not None and len(layer.colors) else layer.style.color)
            for idx, seg in enumerate(verts):
                ax.plot(seg[:, 0], seg[:, 1], seg[:, 2], color=color, linewidth=max(layer.style.line_width, 0.4), label=layer.label if idx == 0 and layer.label else None)
            continue
        if layer.layer_type in {"mesh3d", "bars3d"} and layer.vertices is not None:
            verts = layer.vertices
            color = layer.colors[:, :4] if layer.colors is not None and len(layer.colors) == len(verts) else None
            ssao_opts = getattr(getattr(engine, "options", None), "visual", None)
            ssao_opts = getattr(ssao_opts, "ssao", None)
            ssao_on = bool(getattr(ssao_opts, "enabled", False) or layer.metadata.get("ssao", False))
            ssao_strength = float(layer.metadata.get("ssao_strength", getattr(ssao_opts, "strength", 0.45)))
            if layer.indices is not None and len(layer.indices) >= 3:
                ...
                if face_color is not None and ssao_on:
                    face_color = _apply_preview_ssao(face_color, verts[tris[:, 0], 2], ssao_strength)
                surf = ax.plot_trisurf(
                    verts[:, 0], verts[:, 1], verts[:, 2],
                    triangles=tris,
                    linewidth=0.0 if layer.layer_type == "bars3d" else 0.08,
                    antialiased=True, shade=True,
                    color=None if face_color is not None else _rgba(layer.style.color, (0.1, 0.45, 1.0, 0.8)),
                    alpha=0.92,
                )
                ...
                ax.scatter(verts[:, 0], verts[:, 1], verts[:, 2], c=layer.colors, s=layer.style.point_size, depthshade=False, label=layer.label or None)
            continue
        if layer.metadata.get("artist") == "pcolormesh":
            ax.pcolormesh(layer.metadata["X"], layer.metadata["Y"], layer.metadata["C"], cmap=layer.metadata.get("cmap", "viridis"), shading=layer.metadata.get("shading", "auto"), vmin=layer.metadata.get("vmin"), vmax=layer.metadata.get("vmax"))
            continue
        if layer.metadata.get("artist") == "contourf":
            ax.contourf(layer.metadata["X"], layer.metadata["Y"], layer.metadata["Z"], levels=layer.metadata.get("levels", 10), cmap=layer.metadata.get("cmap", "viridis"), alpha=layer.metadata.get("alpha"))
            continue
        if layer.metadata.get("artist") == "contour":
            contour_kwargs = {"levels": layer.metadata.get("levels", 10), "linewidths": layer.metadata.get("linewidths", 1.0)}
            ...
            ax.contour(layer.metadata["X"], layer.metadata["Y"], layer.metadata["Z"], **contour_kwargs)
            continue
        if layer.metadata.get("artist") == "imshow":
            xmin, xmax, ymin, ymax = layer.metadata["extent"]
            ax.imshow(layer.metadata["matrix"], extent=(xmin, xmax, ymin, ymax), origin=layer.metadata.get("origin", "upper"), cmap=layer.metadata.get("cmap", "viridis"), vmin=layer.metadata.get("vmin"), vmax=layer.metadata.get("vmax"), aspect="auto", interpolation="nearest")
            continue
        if layer.layer_type == "polyline" and layer.pts is not None:
            if layer.metadata.get("artist_group") == "quiver":
                ...
                if valid.any():
                    ...
                    ax.quiver(xs, ys, us, vs, angles="xy", scale_units="xy", scale=1,
                              color=[col], width=max(layer.style.line_width * 0.002, 0.002),
                              headwidth=4, headlength=4, headaxislength=3.5,
                              label=layer.label or None)
            else:
                pts = layer.pts
                ax.plot(pts[:, 0], pts[:, 1], color=_rgba(layer.style.color), linewidth=layer.style.line_width, label=layer.label or None)
            ...
            pts = layer.pts
            ax.scatter(pts[:, 0], pts[:, 1], c=layer.colors, s=layer.style.point_size, label=layer.label or None)
            ...
            if layer.indices is not None and getattr(layer, "mode", "strip") == "triangles":
                ...
                coll = PolyCollection(polys, facecolors=[fc], edgecolors=[ec], linewidths=layer.style.line_width, label=layer.label or None)
                ax.add_collection(coll)
            else:
                patch = Polygon(layer.vertices, closed=True, facecolor=fc, edgecolor=ec, linewidth=layer.style.line_width, label=layer.label or None)
                ax.add_patch(patch)
            ...
            y_edges = np.linspace(float(ymin_v), float(ymax_v), density.shape[0] + 1, dtype=np.float32)
            chunk_size = 8192
            ...
            ax.imshow(np.log1p(density), extent=(float(x0), float(x1), float(ymin_v), float(ymax_v)), origin="lower", aspect="auto", cmap="magma", interpolation="bilinear")
            ax.set_xlim(float(x0), float(x1))
            ax.set_ylim(float(ymin_v), float(ymax_v))
        elif layer.layer_type == "text":
            ax.text(layer.x, layer.y, layer.text, fontsize=layer.style.text_size_px, color=_rgba(layer.style.color))
    ...
    if labels:
        ...
        ax.legend(legend_handles, legend_labels, loc="upper right", fontsize=8, framealpha=0.78, borderpad=0.35, labelspacing=0.3, handlelength=1.4)
