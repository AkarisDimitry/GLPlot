...
def _parse_plot_format(fmt: Optional[str]) -> dict[str, Any]:
    if not fmt:
        return {}
    colors = {"b": "b", "g": "g", "r": "r", "c": "cyan", "m": "magenta", "y": "yellow", "k": "k", "w": "w"}
...
def _project_3d(
    ...
    *,
    ...
    return np.ascontiguousarray(xp, dtype=np.float32), np.ascontiguousarray(yp, dtype=np.float32), z_arr


def _colormap_values(values, cmap: Optional[str] = None, vmin: Optional[float] = None, vmax: Optional[float] = None) -> np.ndarray:
...
def _add_3d_layer(
    ...
    *,
    ...
    layer = Layer3D(verts, colors=np.ascontiguousarray(cols, dtype=np.float32), indices=idx, primitive=primitive, label=label or "", layer_type=layer_type)
...
def _add_plot_primitive(
    ...
    **kwargs,
    ...
    if marker not in (None, "", "None", "none", " "):
        ...
        plot_obj.add_scatter(x_arr, y_arr, _normalize_rgba(tuple(rgba), n=len(x_arr)), float(size or 6.0), label=label)
        layer = plot_obj.scene.layers[-1]
...
def scatter(
    ...
    plot_obj.scene.layers[-1].metadata.update({"marker": marker, "artist": "scatter", "cmap": cmap, "vmin": vmin, "vmax": vmax})
    _set_dirty(plot_obj)
    return plot_obj.scene.layers[-1]


def scatter3d(x, y, z, *args, elev: float = 30.0, azim: float = -60.0, scale_z: float = 0.7, c: Optional[ColorLike] = None, cmap: Optional[str] = None, **kwargs):
    ...
    if values is not None and not isinstance(values, str) and np.asarray(values).ndim == 1 and len(np.asarray(values)) == len(verts):
...
def fill_between(
    ...
    *,
    ...
    y2_arr = np.full_like(y1_arr, float(y2)) if np.isscalar(y2) else _as_float_array(y2, ndim=1, name="y2")
    ...
    if alpha is not None:
        rgba[3] *= float(alpha)
    return add_patch(verts, mode="strip", face_color=tuple(rgba), edge_color=tuple(rgba), label=label)
...
def bar(
    ...
    *,
    ...
    b_arr = np.full_like(h_arr, float(bottom)) if np.isscalar(bottom) else _as_float_array(bottom, ndim=1, name="bottom")
    ...
    for idx, (xc, h, btm) in enumerate(zip(x_arr, h_arr, b_arr)):
        verts = np.array([[xc - half, btm], [xc - half, btm + h], [xc + half, btm + h], [xc + half, btm]], dtype=np.float32)
        patches.append(add_patch(verts, indices=_QUAD_INDICES, mode="triangles", face_color=tuple(rgba), edge_color=tuple(rgba), label=label if idx == 0 else None))
    return patches


def hist(x: Sequence[float], bins: Union[int, Sequence[float]] = 10, *, density: bool = False, color: ColorLike = (0.2, 0.4, 0.8, 1.0), alpha: Optional[float] = None, label: Optional[str] = None):
    values = _as_float_array(x, ndim=1, name="x")
    counts, edges = np.histogram(values, bins=bins, density=density)
    centers = 0.5 * (edges[:-1] + edges[1:])
    artists = bar(centers, counts, width=float(np.min(np.diff(edges))), color=color, alpha=alpha, label=label)
    return counts, edges, artists


def hist2d(x, y, bins=100, range=None, density: bool = False, cmap: str = "magma", s: Optional[float] = None, label: Optional[str] = None):
    x_arr = _as_float_array(x, ndim=1, name="x")
    y_arr = _as_float_array(y, ndim=1, name="y")
    if len(x_arr) != len(y_arr):
        raise ValueError("x and y must have the same length")
    ...
    layer.metadata.update({"artist": "hist2d", "counts": counts, "xedges": xedges, "yedges": yedges})
...
def imshow(
    ...
    **kwargs,
    ...
    ys = np.linspace(ymax, ymin, rows, dtype=np.float32) if origin == "upper" else np.linspace(ymin, ymax, rows, dtype=np.float32)
    ...
    layer = plot_obj.scene.layers[-1]
    layer.metadata.update({"artist": "imshow", "matrix": matrix, "extent": (xmin, xmax, ymin, ymax), "origin": origin, "cmap": cmap, "vmin": vmin, "vmax": vmax})
    _set_dirty(plot_obj)
    return layer
...
def pcolormesh(
    ...
    **kwargs,
    ...
    plot_obj.add_scatter(xx.ravel(), yy.ravel(), colors, float(size), label=label)
    layer = plot_obj.scene.layers[-1]
    layer.metadata.update({"artist": "pcolormesh", "X": xx, "Y": yy, "C": matrix, "cmap": cmap, "shading": shading, "vmin": vmin, "vmax": vmax})
    _set_dirty(plot_obj)
    return layer


def contour(X, Y=None, Z=None, levels=10, *, colors: Optional[ColorLike] = None, cmap: str = "viridis", linewidths: float = 1.0, label: Optional[str] = None, **kwargs):
    if Z is None:
    ...
    layer.metadata.update({"artist": "contour", "X": xx, "Y": yy, "Z": matrix, "levels": levels, "colors": colors, "cmap": cmap, "linewidths": linewidths})
    return layer


def contourf(X, Y=None, Z=None, levels=10, *, cmap: str = "viridis", alpha: Optional[float] = None, label: Optional[str] = None, **kwargs):
    if Z is None:
        matrix = _as_float_array(X, ndim=2, name="Z")
        yy, xx = np.indices(matrix.shape, dtype=np.float32)
    else:
    ...
    layer.metadata.update({"artist": "contourf", "X": xx, "Y": yy, "Z": matrix, "levels": levels, "cmap": cmap, "alpha": alpha})
    return layer


def plot_surface(X, Y, Z, *, cmap: str = "viridis", elev: float = 30.0, azim: float = -60.0, scale_z: float = 0.7, rstride: int = 1, cstride: int = 1, alpha: Optional[float] = None, label: Optional[str] = None, **kwargs):
    xx = _as_float_array(X, name="X")
    yy = _as_float_array(Y, name="Y")
    ...
    rows, cols = xs.shape
    verts = np.column_stack([xs.ravel(), ys.ravel(), (zs * float(scale_z)).ravel()]).astype(np.float32)
    ...
    layer = _add_3d_layer(
        ...
        elev=elev,
        azim=azim,
        metadata={"artist": "plot_surface", "X": xx, "Y": yy, "Z": zz, "scale_z": scale_z, "cmap": cmap, "rstride": rstride, "cstride": cstride},
    )
    return layer


def mesh3d(vertices, faces=None, *, color: ColorLike = "tab:blue", c: Optional[Sequence[float]] = None, cmap: str = "viridis", alpha: Optional[float] = None, elev: float = 30.0, azim: float = -60.0, label: Optional[str] = None):
    verts = _as_float_array(vertices, ndim=2, name="vertices")
    if verts.shape[1] != 3:
...
def volume3d(x, y, z, values=None, *, threshold: Optional[float] = None, cmap: str = "magma", alpha: float = 0.45, elev: float = 30.0, azim: float = -60.0, s: float = 2.0, label: Optional[str] = None):
    ...
    layer = _add_3d_layer(
        ...
        azim=azim,
        point_size=s,
        metadata={"artist": "volume3d", "threshold": threshold, "cmap": cmap, "values": val_arr[mask]},
    )
    return layer
...
def plot_wireframe(X, Y, Z, *, elev: float = 30.0, azim: float = -60.0, scale_z: float = 0.7, rstride: int = 4, cstride: int = 4, color: ColorLike = "k", linewidth: float = 0.7, label: Optional[str] = None, **kwargs):
    xx = _as_float_array(X, name="X")
    yy = _as_float_array(Y, name="Y")
    zz = _as_float_array(Z, name="Z")
    if not (xx.shape == yy.shape == zz.shape):
    ...
    for row in range(0, xx.shape[0], max(1, int(rstride))):
        pts = np.column_stack([xx[row, :], yy[row, :], zz[row, :] * float(scale_z)]).astype(np.float32)
        segments.extend(np.column_stack([pts[:-1], pts[1:]]).reshape(-1, 3))
    for col in range(0, xx.shape[1], max(1, int(cstride))):
        pts = np.column_stack([xx[:, col], yy[:, col], zz[:, col] * float(scale_z)]).astype(np.float32)
        segments.extend(np.column_stack([pts[:-1], pts[1:]]).reshape(-1, 3))
    layer = _add_3d_layer(
        ...
        azim=azim,
        color=color,
        metadata={"artist": "plot_wireframe", "X": xx, "Y": yy, "Z": zz, "scale_z": scale_z, "rstride": rstride, "cstride": cstride},
    )
...
def arrow(x, y, dx, dy, *, width: float = 1.0, head_width: float = 0.12, head_length: float = 0.18, color: ColorLike = "k", alpha: Optional[float] = None, label: Optional[str] = None, **kwargs):
    rgba = list(_normalize_rgba(color, n=None))
    if alpha is not None:
        rgba[3] *= float(alpha)
...
def quiver(x, y, u, v, *, color: ColorLike = "k", scale: float = 1.0, width: float = 1.0, head_width: float = 0.08, head_length: float = 0.12, label: Optional[str] = None, **kwargs):
    x_arr = _as_float_array(x, name="x").ravel()
    y_arr = _as_float_array(y, name="y").ravel()
    ...
    if valid.any():
        ...
        add_patch(head_verts, indices=head_indices, mode="triangles", face_color=rgba, edge_color=rgba)
...
def quiver3d(
    ...
    **kwargs,
    ...
    layer = _add_3d_layer(
        ...
        azim=azim,
        color=color,
        metadata={"artist": "quiver3d", "vector_count": int(len(origins)), "normalize": bool(normalize)},
    )
    layer.style.line_width = float(linewidth)
    return [layer]


def annotate(text_value: str, xy, xytext=None, arrowprops: Optional[dict[str, Any]] = None, fontsize: int = 12, color: ColorLike = "k", **kwargs):
    ...
    if xytext is not None and arrowprops is not None:
        ...
        artists.extend(arrow(tx, ty, float(xy[0]) - float(tx), float(xy[1]) - float(ty), color=arr_color, **props))
    return artists


def hlines(y, xmin, xmax, colors: ColorLike = "k", linestyles: str = "-", label: Optional[str] = None, linewidth: float = 1.0, **kwargs):
    ...
    for idx, (yy, x0, x1) in enumerate(zip(y_arr, xmin_arr, xmax_arr)):
        artists.extend(plot([x0, x1], [yy, yy], color=colors, linestyle=linestyles, linewidth=linewidth, label=label if idx == 0 else None))
        artists[-1].metadata["artist"] = "hline"
    return artists


def vlines(x, ymin, ymax, colors: ColorLike = "k", linestyles: str = "-", label: Optional[str] = None, linewidth: float = 1.0, **kwargs):
    ...
    for idx, (xx, y0, y1) in enumerate(zip(x_arr, ymin_arr, ymax_arr)):
        artists.extend(plot([xx, xx], [y0, y1], color=colors, linestyle=linestyles, linewidth=linewidth, label=label if idx == 0 else None))
        artists[-1].metadata["artist"] = "vline"
    return artists


def axhline(y: float = 0.0, xmin: float = 0.0, xmax: float = 1.0, color: ColorLike = "k", linestyle: str = "-", linewidth: float = 1.0, label: Optional[str] = None, **kwargs):
    left, right = _get_or_create_plot().get_xlim()
    x0 = left + (right - left) * float(xmin)
    x1 = left + (right - left) * float(xmax)
    artists = hlines(float(y), x0, x1, colors=color, linestyles=linestyle, linewidth=linewidth, label=label)
    for artist in artists:
        artist.metadata.update({"guide": True, "artist": "axhline"})
    return artists[0] if artists else None


def axvline(x: float = 0.0, ymin: float = 0.0, ymax: float = 1.0, color: ColorLike = "k", linestyle: str = "-", linewidth: float = 1.0, label: Optional[str] = None, **kwargs):
    bottom, top = _get_or_create_plot().get_ylim()
    y0 = bottom + (top - bottom) * float(ymin)
    y1 = bottom + (top - bottom) * float(ymax)
    artists = vlines(float(x), y0, y1, colors=color, linestyles=linestyle, linewidth=linewidth, label=label)
    for artist in artists:
        artist.metadata.update({"guide": True, "artist": "axvline"})
    return artists[0] if artists else None


def axline(xy1, xy2=None, *, slope: Optional[float] = None, color: ColorLike = "k", linestyle: str = "-", linewidth: float = 1.0, label: Optional[str] = None, **kwargs):
    x0, y0 = map(float, xy1)
    if xy2 is not None:
        x1, y1 = map(float, xy2)
        if abs(x1 - x0) < 1e-12:
    ...
    artists = plot([left, right], [y_left, y_right], color=color, linestyle=linestyle, linewidth=linewidth, label=label)
...
def errorbar(x, y, yerr=None, xerr=None, fmt: str = "", ecolor: Optional[ColorLike] = None, elinewidth: float = 1.0, capsize: float = 0.0, label: Optional[str] = None, **kwargs):
    x_arr = _as_float_array(x, ndim=1, name="x")
    y_arr = _as_float_array(y, ndim=1, name="y")
    if len(x_arr) != len(y_arr):
        raise ValueError("x and y must have the same length")
    ...
    if yerr is not None:
        ...
        for xx, yy, ee in zip(x_arr, y_arr, yerr_arr):
            ...
            if capsize:
                artists.extend(hlines([yy - ee, yy + ee], xx - capsize, xx + capsize, colors=err_color, linewidth=elinewidth))
    if xerr is not None:
        ...
        for xx, yy, ee in zip(x_arr, y_arr, xerr_arr):
            ...
            if capsize:
                artists.extend(vlines([xx - ee, xx + ee], yy - capsize, yy + capsize, colors=err_color, linewidth=elinewidth))
...
def stem(x, y=None, linefmt: str = "C0-", markerfmt: str = "C0o", basefmt: str = "k-", bottom: float = 0.0, label: Optional[str] = None):
    if y is None:
        y_arr = _as_float_array(x, ndim=1, name="y")
        x_arr = np.arange(len(y_arr), dtype=np.float32)
    else:
    ...
    artists.append(scatter(x_arr, y_arr, color=marker_style.get("color", color), marker=marker_style.get("marker", "o")))
